...
export const loadUser = () => async (dispatch) => {
  try {
    ...
    dispatch({ type: LOAD_USER_SUCCESS, payload: data.user || null });
  } catch (error) {
    if (error.response) {
      ...
      dispatch({ type: LOAD_USER_FAIL, payload: 'No response received from the server.' });
    } else {
      // Something happened in setting up the request that triggered an error
      dispatch({ type: LOAD_USER_FAIL, payload: 'Error occurred while sending the request.' });
    }
  }
};
